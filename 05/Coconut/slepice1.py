...
@_coconut_tco  # line 2
def hash_generator(code, salt=0):  # line 4
    hash_ = hashlib.md5(code + bytes(str(salt), 'utf-8')).hexdigest()  # line 5
    if hash_[:5] == "00000":  # line 6
        raise _coconut_tail_call(_coconut.itertools.chain.from_iterable, (_coconut_lazy_item() for _coconut_lazy_item in (lambda: (hash_,), lambda: hash_generator(code, salt + 1))))  # line 7
    else:  # line 8
        raise _coconut_tail_call(hash_generator, code, salt + 1)  # line 9

zero_starting_hashes = hash_generator(b"uqwqemis")  # line 11
password = (list)((_coconut.functools.partial(map, lambda _: _[5]))(_coconut_igetitem(zero_starting_hashes, _coconut.slice(None, 8))))  # line 12
print("".join(password))  # line 13
