...
@recursive_iterator  # line 2
@_coconut_tco  # line 2
def hash_generator(code, found_indexes, salt=0):  # line 4
    hash_ = hashlib.md5(code + bytes(str(salt), 'utf-8')).hexdigest()  # line 5
    if not hash_[5].isdigit():  # line 6
        raise _coconut_tail_call(hash_generator, code, found_indexes, salt + 1)  # line 7
    pass_pos = int(hash_[5])  # line 8
    if (hash_[:5] == "00000" and 0 <= pass_pos < 8 and pass_pos not in found_indexes):  # line 9
        raise _coconut_tail_call(_coconut.itertools.chain.from_iterable, (_coconut_lazy_item() for _coconut_lazy_item in (lambda: (hash_,), lambda: hash_generator(code, found_indexes | set((pass_pos,)), salt + 1))))  # line 12
    else:  # line 13
        raise _coconut_tail_call(hash_generator, code, found_indexes, salt + 1)  # line 14

zero_starting_hashes = hash_generator(b"uqwqemis", set())  # line 16
password = (list)((_coconut.functools.partial(map, lambda _=None: _[6]))((_coconut.functools.partial(sorted, key=lambda _=None: int(_[5])))(_coconut_igetitem(zero_starting_hashes, _coconut.slice(None, 8)))))  # line 17
print("".join(password))  # line 18
