...
class _coconut(object):
    ...
    IndexError, NameError, ValueError, map, zip, filter, dict, frozenset, getattr, hasattr, hash, isinstance, iter, len, list, min, next, object, range, reversed, set, slice, super, tuple, bytearray, repr = IndexError, NameError, ValueError, map, zip, filter, dict, frozenset, getattr, hasattr, hash, isinstance, iter, len, list, min, next, object, range, reversed, set, slice, super, tuple, bytearray, staticmethod(repr)
...
class _coconut_filter(_coconut.filter):
...
MatchError, map, reduce, filter, takewhile, dropwhile, tee = _coconut_MatchError, _coconut_map, _coconut.functools.reduce, _coconut_filter, _coconut.itertools.takewhile, _coconut.itertools.dropwhile, _coconut_tee
...
def can_be_constructed(*_coconut_match_to): # line 3
    _coconut_match_check = False # line 3
    if (_coconut.len(_coconut_match_to) == 1) and (_coconut.isinstance(_coconut_match_to[0], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to[0]) == 3): # line 3
        x = _coconut_match_to[0][0] # line 3
        y = _coconut_match_to[0][1] # line 3
        z = _coconut_match_to[0][2] # line 3
        _coconut_match_check = True # line 3
    if not _coconut_match_check: # line 3
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def can_be_constructed((x, y, z)) = x + y > z'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to))) # line 3
        _coconut_match_err.pattern = 'def can_be_constructed((x, y, z)) = x + y > z' # line 3
        _coconut_match_err.value = _coconut_match_to # line 3
        raise _coconut_match_err # line 3

    return x + y > z # line 4

pinstr = ((_coconut.functools.partial(map, lambda x: [int(x[i:i + 5]) for i in range(0, 11, 5)]))(instructs.splitlines())) # line 6

transformed_instructs = ((_coconut.functools.partial(map, sorted))((_coconut.functools.partial(reduce, _coconut.operator.add))((_coconut.functools.partial(map, lambda i: (list)(zip(pinstr[i], pinstr[i + 1], pinstr[i + 2]))))(range(0, len(pinstr), 3))))) # line 9

(print)((reversed)((_coconut.functools.partial(filter, can_be_constructed))(transformed_instructs))) # line 15
