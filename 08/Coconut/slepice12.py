...
DISPLAY_WIDTH = 50  # line 1
DISPLAY_HEIGHT = 6  # line 2

def shift(line, amount):  # line 4
    amount %= len(line)  # line 5
    return line[len(line) - amount:] + line[:len(line) - amount]  # line 6

def exec_instructions(*_coconut_match_to_args, **_coconut_match_to_kwargs):  # line 7
    _coconut_match_check = False  # line 7
    if (_coconut.len(_coconut_match_to_args) == 2) and ("display" not in _coconut_match_to_kwargs) and (_coconut.isinstance(_coconut_match_to_args[1], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to_args[1]) == 0):  # line 7
        _coconut_match_temp_0 = _coconut_match_to_args[0] if _coconut.len(_coconut_match_to_args) > 0 else _coconut_match_to_kwargs.pop("display")  # line 7
        if (not _coconut_match_to_kwargs):  # line 7
            display = _coconut_match_temp_0  # line 7
            _coconut_match_check = True  # line 7
    if not _coconut_match_check:  # line 7
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def exec_instructions(display, []) = display'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))  # line 7
        _coconut_match_err.pattern = 'def exec_instructions(display, []) = display'  # line 7
        _coconut_match_err.value = _coconut_match_to_args  # line 7
        raise _coconut_match_err  # line 7

    return display  # line 8

@addpattern(exec_instructions)  # line 9
@_coconut_tco  # line 9
def exec_instructions(*_coconut_match_to_args, **_coconut_match_to_kwargs):  # line 9
    _coconut_match_check = False  # line 9
    if (_coconut.len(_coconut_match_to_args) == 2) and ("display" not in _coconut_match_to_kwargs) and (_coconut.isinstance(_coconut_match_to_args[1], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to_args[1]) >= 1):  # line 9
        _coconut_match_temp_0 = _coconut_match_to_args[0] if _coconut.len(_coconut_match_to_args) > 0 else _coconut_match_to_kwargs.pop("display")  # line 9
        rest = _coconut.list(_coconut_match_to_args[1][1:])  # line 9
        instruction = _coconut_match_to_args[1][0]  # line 9
        if (not _coconut_match_to_kwargs):  # line 9
            display = _coconut_match_temp_0  # line 9
            _coconut_match_check = True  # line 9
    if not _coconut_match_check:  # line 9
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def exec_instructions(display, [instruction] + rest):'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))  # line 9
        _coconut_match_err.pattern = 'def exec_instructions(display, [instruction] + rest):'  # line 9
        _coconut_match_err.value = _coconut_match_to_args  # line 9
        raise _coconut_match_err  # line 9

    if instruction[:4] == "rect":  # line 12
        width, height = (_coconut.functools.partial(map, int))(instruction[5:].split("x"))  # line 13
        display = [["█" if width > i else display[j][i] for i in range(DISPLAY_WIDTH)] if height > j else display[j] for j in range(DISPLAY_HEIGHT)]  # line 14
    else:  # line 15
        pos, amount = instruction.split(" by ")  # line 16
        pos, amount = int(pos.split("=")[1]), int(amount)  # line 17
        if instruction[7:10] == "row":  # line 18
            display[pos] = shift(display[pos], amount)  # line 19
        else:  # line 20
            column = [display[i][pos] for i in range(DISPLAY_HEIGHT)]  # line 21
            column = shift(column, amount)  # line 22
            display = [[column[j] if pos == i else display[j][i] for i in range(DISPLAY_WIDTH)] for j in range(DISPLAY_HEIGHT)]  # line 23

    raise _coconut_tail_call(exec_instructions, display, rest)  # line 25


with open("slepice.txt", "r") as f :  # line 28
    instructions = f.read()  # line 29

display = [[" " for _ in range(50)] for i in range(6)]  # line 31
display = (_coconut.functools.partial(exec_instructions, display))(instructions.splitlines())  # line 32

print("\n".join(["".join(line) for line in display]))  # line 34
print(sum(("".join(line).count("█") for line in display)))  # line 35
