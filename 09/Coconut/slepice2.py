...
def get_mark_info(compressed):  # line 1
    mark_end = compressed.find(")")  # line 2
    mark = compressed[1:mark_end]  # line 3
    n, repeats = (_coconut.functools.partial(map, int))(mark.split('x'))  # line 4
    return (mark_end, n, repeats)  # line 5

def decompress(*_coconut_match_to_args, **_coconut_match_to_kwargs):  # line 6
    _coconut_match_check = False  # line 6
    if (_coconut.len(_coconut_match_to_args) == 1) and (_coconut_match_to_args[0] == ""):  # line 6
        if (not _coconut_match_to_kwargs):  # line 6
            _coconut_match_check = True  # line 6
    if not _coconut_match_check:  # line 6
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " '\'def decompress("") = 0\'' " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))  # line 6
        _coconut_match_err.pattern = 'def decompress("") = 0'  # line 6
        _coconut_match_err.value = _coconut_match_to_args  # line 6
        raise _coconut_match_err  # line 6

    return 0  # line 7

@addpattern(decompress)  # line 8
def decompress(*_coconut_match_to_args, **_coconut_match_to_kwargs):  # line 8
    _coconut_match_check = False  # line 8
    if (_coconut.len(_coconut_match_to_args) <= 1) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 0, "compressed" in _coconut_match_to_kwargs)) == 1):  # line 8
        _coconut_match_temp_0 = _coconut_match_to_args[0] if _coconut.len(_coconut_match_to_args) > 0 else _coconut_match_to_kwargs.pop("compressed")  # line 8
        if (not _coconut_match_to_kwargs):  # line 8
            compressed = _coconut_match_temp_0  # line 8
            _coconut_match_check = True  # line 8
    if _coconut_match_check and not (compressed[0] == "("):  # line 8
        _coconut_match_check = False  # line 8
    if not _coconut_match_check:  # line 8
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " '\'def decompress(compressed if compressed[0] == "("):\'' " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))  # line 8
        _coconut_match_err.pattern = 'def decompress(compressed if compressed[0] == "("):'  # line 8
        _coconut_match_err.value = _coconut_match_to_args  # line 8
        raise _coconut_match_err  # line 8

    mark_end, n, repeats = get_mark_info(compressed)  # line 11
    decompressed_len = decompress(compressed[mark_end + 1:mark_end + n + 1]) * repeats  # line 12
    pos = mark_end + n + 1  # line 13
    return decompressed_len + decompress(compressed[pos:])  # line 14

@addpattern(decompress)  # line 15
def decompress(compressed):  # line 15
    return 1 + decompress(compressed[1:])  # line 17

with open("slepice.txt", "r") as f :  # line 19
    compressed = f.read().strip()  # line 20

(print)(decompress(compressed))  # line 22
