...
with open("slepice.txt", "r") as f :  # line 3
    rooms = f.read()  # line 4

@recursive_iterator  # line 5
def parse_rooms(*_coconut_match_to_args, **_coconut_match_to_kwargs):  # line 5
    _coconut_match_check = False  # line 5
    if (_coconut.len(_coconut_match_to_args) == 1) and (_coconut.isinstance(_coconut_match_to_args[0], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to_args[0]) == 0):  # line 5
        if (not _coconut_match_to_kwargs):  # line 5
            _coconut_match_check = True  # line 5
    if not _coconut_match_check:  # line 5
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def parse_rooms([]) = (0,)'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))  # line 5
        _coconut_match_err.pattern = 'def parse_rooms([]) = (0,)'  # line 5
        _coconut_match_err.value = _coconut_match_to_args  # line 5
        raise _coconut_match_err  # line 5

    return (0,)  # line 7

_coconut_decorator_0 = _coconut_compose(recursive_iterator, addpattern(parse_rooms))  # line 8
@_coconut_decorator_0  # line 8
@_coconut_tco  # line 8
def parse_rooms(*_coconut_match_to_args, **_coconut_match_to_kwargs):  # line 8
    _coconut_match_check = False  # line 8
    if (_coconut.len(_coconut_match_to_args) == 1) and (_coconut.isinstance(_coconut_match_to_args[0], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to_args[0]) >= 1):  # line 8
        rest = _coconut.list(_coconut_match_to_args[0][1:])  # line 8
        room = _coconut_match_to_args[0][0]  # line 8
        if (not _coconut_match_to_kwargs):  # line 8
            _coconut_match_check = True  # line 8
    if not _coconut_match_check:  # line 8
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def parse_rooms([room] + rest):'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))  # line 8
        _coconut_match_err.pattern = 'def parse_rooms([room] + rest):'  # line 8
        _coconut_match_err.value = _coconut_match_to_args  # line 8
        raise _coconut_match_err  # line 8

    index = room.find("[")  # line 11
    id_ = int(room[index - 3:index])  # line 12
    checksum = room[index + 1:index + 6]  # line 13
    code = room[:index - 3].replace("-", "")  # line 14
    counter = ((_coconut.functools.partial(map, lambda _=None: _[0]))((_coconut.functools.partial(sorted, key=lambda _=None: (100 - _[1], _[0])))((_coconut.operator.methodcaller("most_common"))(Counter(code)))))  # line 15
    if checksum == "".join(_coconut_igetitem(counter, _coconut.slice(None, 5))):  # line 18
        raise _coconut_tail_call(_coconut.itertools.chain.from_iterable, (_coconut_lazy_item() for _coconut_lazy_item in (lambda: (id_,), lambda: parse_rooms(rest))))  # line 19
    else:  # line 20
        raise _coconut_tail_call(parse_rooms, rest)  # line 21

(print)((sum)(parse_rooms(rooms.splitlines())))  # line 23
